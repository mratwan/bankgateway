...
class GatewayServiceProviderLaravel5 extends ServiceProvider
{
	...
	public function boot()
	{
        ...
        //php artisan vendor:publish --provider=MrAtwan\BankGateway\GatewayServiceProvider --tag=config
        $this->publishes([
            $config => config_path('gateway.php'),
        ], 'config');

        // php artisan vendor:publish --provider=MrAtwan\BankGateway\GatewayServiceProvider --tag=migrations
        $this->publishes([
        ...
        // php artisan vendor:publish --provider=MrAtwan\BankGateway\GatewayServiceProvider --tag=views
        $this->publishes([
